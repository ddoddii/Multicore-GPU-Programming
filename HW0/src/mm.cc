...
void MatchMaker::Match() {
    // TO-DO : Good Luck!
    // WARNING: You must not copy answer from answer file!
    ...
    // You should store the answer in 'output' array

    /* Code Start */
    for (int i = 0; i<output_len; i++) {
	    int count = 0;
	    string queryStr(query[i], query_len[i]);
	    size_t pos = string(ref_str).find(queryStr,0);

	    while (pos != string::npos) {
		    count ++;
		    pos = string(ref_str).find(queryStr, pos+1);
	    }
	    output[i] = count;
    }
